...
# Objective and gradient function for Optim: computes both simultaneously
# x is in physical space (no normalization)
function f_and_g!(F, G, x)
    iteration_count[] += 1
    ...
    # Update F and G for Optim (if requested)
    if G !== nothing
        G .= grad_regularized
    end
...
# fd_gradient = FiniteDiff.finite_difference_gradient(x -> f_and_g!(0.0, nothing, x), x0)
...
# Redirect stdout to log file to capture Optim trace output
original_stdout = stdout
redirect_stdout(log_file_handle[])

try
    # Run optimization - Optim trace output will go to log file
    # Use Fminbox for bounded L-BFGS optimization with more exploratory settings
    global result
    result = optimize(
        Optim.only_fg!(f_and_g!),
        lower_bounds,
        upper_bounds,
        x0,
        Fminbox(LBFGS(
        ...
        Optim.Options(
            ...
            show_trace=true,       # Print iteration info to console
            extended_trace=true,   # Include extra info (x, g(x), metadata)
            store_trace=true,      # Store trace in result object
            show_every=1,          # Show every iteration
            g_tol=1e-6,            # Projected gradient tolerance
...
println("Gradient evaluations: $(Optim.g_calls(result))")
println()
...
results_file = joinpath(vis_dir, "optimization_results_$(rad2deg(desired_direction_angle))_deg.jld2")
jldsave(results_file;
    # Optimized parameters
    B_prep_opt = B_prep_opt,
    ψ_tail_opt = ψ_tail_opt,
    φ_opt = φ_opt,
    xout = xout,
    fout = fout,

    # Initial parameters
    B_prep_init = B_prep_init,
    ψ_tail_init = ψ_tail_init,
    φ_init = φ_init,
    x0 = x0,
    ...
    tikhonov_lambda_smooth = tikhonov_lambda_smooth,

    # Fixed parameters
    K_prop_fixed = K_prop_fixed,
    T_prep = T_prep,
    T_prop = T_prop,
    desired_direction_angle = desired_direction_angle,
    ...
    # Final trajectories from cache
    final_trajectories = sim_cache.trajectories,
